...
class AddToBookmarkWindow_main(QDialog, form_class_main):
    ...
    def refresh_bookmark_list(self):
        bookmark_row = gl.db.executeOneQueryMany("SELECT * FROM BOOKMARK")  # 북마크 전체 불러옴
...
class AddToBookmarkWindow_new(QDialog, form_class_new):
    ...
    def accept(self):
        ...
        print("2")
        row = gl.db.executeOneQuery("SELECT COUNT(*) FROM BOOKMARK WHERE NAME is '" + bookmark_name + "'")[0]
        print("3")
        if row == 0:
            item_dict = {}
            item_dict['num'] = 1
            item_dict['name'] = bookmark_name
            gl.db.insertDB("BOOKMARK", item_dict)
            gl.db.commit()
            self.close()
