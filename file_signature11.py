...
def is_bad_signature(file_name_list):
    ...
    conn = sqlite3.connect("./db/file_signature11.db")# 파일 시그니처 들어가있는 DB
    cur = conn.cursor()
    ...
    for index in range(len(db_row)):
        ...
        if (db_row[index][footer_signature_field] != "none"):
            flist = [db_row[index][footer_signature_field][i:i + 2] for i in
                     range(0, len(db_row[index][footer_signature_field]), 2)]
        else:
            pass_footer_key = 1
        ...
        while ('0x' + hlist[i]) == format(struct.unpack_from(">B", sp, 0x0 + i)[0], '#04x') or hlist[i] == '??':
            i += 1
            ...
            if (i+1 == int(len(db_row[index][header_signature_field])) / 2):
                ...
                k = 0
                while ('0x' + flist[k]) == format(struct.unpack_from(">B", sp, 0x0 + k)[0], '#04x') or flist[k] == "??":
...
#####!!!TEST!!!#####
file_name_list = ['C:/Users/조재민/Desktop/출결/009-pen-copy.png']
