...
class DB:
    ...
    def executeOneQueryWithDict(self, sql,
                                item_dict):  # dictionary 붙을때 SQL실행시켜주는 함수 executeOneQueryWithDict("SELECT * FROM asdas WHERE NAME IS :ASdfasdf", item_dict)
        try:
            self.sql = sql
            self.cur.execute(self.sql, item_dict)
            self.conn.commit()
    ...
    def executeOneQuery(self,
                        sql):  # dictionary 안붙을떄 SQL실행시켜주는 함수 executeOneQuery("SELECT * FROM asdf WHERE NAME IS ASDF")
        try:
            dbgmsg("sql : " + sql)
            self.sql = sql
            self.cur.execute(self.sql)
