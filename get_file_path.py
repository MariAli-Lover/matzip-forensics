    ...
    for (dirpath, dirnames, filenames) in os.walk(input_mypath):
        ...
        for items in filenames:
            ...
            if os.path.isfile(one_item):  # 파일이면, itemList에 집어넣을 것
                ...
                #folder_num = folder_num_dict[os.path.basename(one_item)]
                ...
                db1.updateDB("EVIDENCE", file_inform)
        for dirs in dirnames:
            #main.printDebugMessage("폴더 탐지")
            folder_num += 1; #폴더 인덱싱
            folder_num_dict[dirpath + "\\" + os.path.basename(dirs)] = folder_num #폴더 인덱싱,
            #upper_folder_num = folder_num_dict[dirpath] #상위폴더의 folder_num을 가져옴
            upper_folder_num = 0
            ...
            item_dict['upper_num'] = upper_folder_num # db 업데이트를 위한 튜플
            db1.updateDB("FOLDER", item_dict)
