...
class MainWindow(QMainWindow, form_class):
    ...
    def ls_UI(self, QModelIndex):
        ...
        # 경로로 폴더를 찾음
        folder_location = gl.db.executeOneQuery(
            "SELECT * FROM FOLDER WHERE PATH is '/" + path + "' AND NAME is '" + str(QModelIndex.data()) + "'")[0]
        row_folder, row_evidences = gl.db.getItemList(folder_location)
...
def showDirectoryTree_UI():
    row_dict = {}
    folders = gl.db.executeOneQueryMany("SELECT * FROM FOLDER")
    ...
    for folder in folders:
        if root_folder_flag == True:  # 최상위 폴더일 때
            ...
            row.setIcon(0, folder_icon)
            row_dict[
                folder[0]] = row  # row_dict에 자신의 폴더넘버로 row 추가item_0 = QtWidgets.QTreeWidgetItem(mainWindow.treeWidget)
            row_dict[folder[3]].addChild(row)  # 상위 row에 자신을 추가


def showBookmark_UI():
    row_dict = {}
    bookmarks = gl.db.executeOneQueryMany("SELECT * FROM BOOKMARK")
